...
<cfif result.success and (not isDefined("SESSION.lastLotLocation") or SESSION.lastLotLocation is not "#input.ci#,#input.cj#")>
    ...
    <cfset result['errorMessage'] = "lot not biddable" />
</cfif>

<cfif result.success and lotRec.ownerId is SESSION.userId>
	<cfset result['success'] = false />
...
<cfif result.success>
	<cfset result = g_lot.bidLot(lotRec,input.bid,SESSION.userId) />
</cfif>
